...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(dtype=tf.uint8,
                                shape=[None, img_height, img_width, nchannels* config.state_history],
                                name='state'
                                )
        ...
        self.done_mask = tf.placeholder(dtype=tf.bool, shape=[None, ], name='done_mast')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = layers.fully_connected(out, num_actions, scope=scope, reuse=reuse, activation_fn=None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        variables_q = tf.get_collection(tf.GraphKeys.VARIABLES, scope=q_scope)
        variables_target_q = tf.get_collection(tf.GraphKeys.VARIABLES, scope=target_q_scope)
        assign_ops = [tf.assign(variables_target_q[i], variables_q[i]) for i in range(len(variables_q))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_sample = self.r + config.gamma* tf.reduce_max(target_q, axis=1)
        q_sample = tf.where(self.done_mask, self.r, q_sample)
        a = tf.one_hot(self.a, num_actions)
        q = tf.reduce_sum(q * a, axis= 1)
        self.loss  = tf.reduce_sum(tf.square(q_sample - q))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        if config.grad_clip:
            grads_and_vars = [[tf.clip_by_norm(gv[0], config.clip_val), gv[1]]
